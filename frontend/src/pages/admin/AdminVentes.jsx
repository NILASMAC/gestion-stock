...
export default function AdminVentes() {
  ...
  const validerVente = async (id) => {
    await fetch(`/api/admin/ventes/${id}/valider`, {
      method: 'PUT',
  ...
  const rejeterVente = async (id) => {
    await fetch(`/api/admin/ventes/${id}/rejeter`, {
      method: 'PUT',
      headers: { 'Authorization': `Bearer ${getToken()}` }
    })
    load()
  ...
  const telechargerExcel = async () => {
    try {
      const response = await fetch('/api/gerant/export-excel', {
        headers: { 'Authorization': `Bearer ${getToken()}` }
      })
      if (!response.ok) { alert("Aucune vente enregistrée aujourd'hui"); return }
